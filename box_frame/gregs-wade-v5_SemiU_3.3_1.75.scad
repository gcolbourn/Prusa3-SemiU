...
include<inc/extruder-configuration.scad>
...
elevation = 15;
wade_block_height=55+elevation+3;
wade_block_width=24;
wade_block_depth=28;
...
// Open 30° slot for idler screws to avoid overhang at top corner
			...
			//get rid of piece floating in the air
			translate([-7,-5,-5])
			cube([14,10,50],center=true);
			}
...
//	translate([hole*(extruder_recess_d/2-1.5),3+1.5,-wade_block_depth/2-1])
//	cylinder(r=1.5,h=wade_block_depth+2,$fn=10);
	translate([hole*(extruder_recess_d/2+0.2),5+2.5,-wade_block_depth/2-1])
	cylinder(r=2.7,h=wade_block_depth+2,$fn=10);
}
